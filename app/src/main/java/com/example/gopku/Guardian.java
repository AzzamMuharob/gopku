...
public class Guardian extends ListActivity {
    @Override
    ...
    private void tampilkanpilihan(String pilihan) {
        try{
            ...
            if (pilihan.equals("Call Center")){
                String notel = "tel:0761571483";
                a = new Intent(Intent.ACTION_DIAL, Uri.parse(notel));
            }
            else if (pilihan.equals("SMS Center")){
                String smstext = "Halo, saya azzam";
                a = new Intent(Intent.ACTION_VIEW);
                a.setData(Uri.parse("sms:0761571483"));
                a.putExtra("sms_body",smstext);
            }
            else if (pilihan.equals("Driving Direction")){
                String lokasi = "google.navigation:q=0.4998911,101.4183762";
                a = new Intent(Intent.ACTION_VIEW, Uri.parse(lokasi));
            }
            else if (pilihan.equals("Website")){
                String website = "https://guardianindonesia.co.id/";
                a = new Intent(Intent.ACTION_VIEW, Uri.parse(website));
            }
            else if (pilihan.equals("Info Google")){
                a = new Intent(Intent.ACTION_WEB_SEARCH);
                a.putExtra(SearchManager.QUERY, "Guardian Pekanbaru");
            }
            startActivity(a);
        } catch (Exception e){
