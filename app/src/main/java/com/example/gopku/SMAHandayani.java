...
public class SMAHandayani extends ListActivity {
    @Override
    ...
    private void tampilkanpilihan(String pilihan) {
        try{
            ...
            if (pilihan.equals("Call Center")){
                String notel = "tel:076131274";
                a = new Intent(Intent.ACTION_DIAL, Uri.parse(notel));
            }
            else if (pilihan.equals("SMS Center")){
                String smstext = "Halo, saya azzam";
                a = new Intent(Intent.ACTION_VIEW);
                a.setData(Uri.parse("sms:076131274"));
                a.putExtra("sms_body",smstext);
            }
            else if (pilihan.equals("Driving Direction")){
                String lokasi = "google.navigation:q=0.5243307,101.4554931";
                a = new Intent(Intent.ACTION_VIEW, Uri.parse(lokasi));
            }
            else if (pilihan.equals("Website")){
                String website = "http://www.smahandayanipekanbaru.sch.id/";
